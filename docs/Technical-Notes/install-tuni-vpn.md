...
Follow the instructions of your University to install. Below are some generate advices.

 * The official help page of [Tampere Uni](https://www.tuni.fi/en/it-services/handbook/networks/remote-access-and-vpn)


## Self-maintained Linux Box (tested Ubuntu 18.04)

eduVPN application is not available for Linux and thefore you must use openvpn.
...
Generate your own at https://eduvpn.MYUNI.fi/vpn-user-portal/configurations (give name, e.g., "joni-laptop", and store somewhere).
...
**Note:** You need to update the VPN configuration files every now and then (the expriry date by default is in the filename, e.g., "Downloads/eduvpn.MYUNI.fi_internet_20201102_joni-laptop.ovpn#)
...
```
Open the eduvpn-client from the terminal (or app launcher) and add the university name from the drop-down menu. They may ask you to sign-in with the MYUNI username. Then select the university name and toggle the connected button. If it does not work then you are on your own. 
